  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FAFAF8] to-[#F5F3F0]">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-4 sm:py-5">
          <div className="flex items-center justify-between gap-4">
            <Link to="/" className="text-2xl sm:text-3xl font-bold text-[var(--color-primary-dark)] flex-shrink-0">
              FreshSource
            </Link>
            <nav className="hidden md:flex items-center gap-6 sm:gap-8 text-sm font-medium flex-1 justify-center">
              ...
              <Link to="/marketplace" className="text-gray-600 hover:text-[var(--color-primary)] transition-colors">
                Marketplace
              </Link>
              <span className="pb-2 border-b-2 border-[var(--color-primary)] text-[var(--color-primary)]">Reviews</span>
              <Link to="/dashboard" className="text-gray-600 hover:text-[var(--color-primary)] transition-colors">
                Dashboard
              </Link>
            </nav>
            <Link to="/auth" className="text-xs sm:text-sm font-semibold text-[var(--color-primary)] hover:text-[var(--color-primary-dark)] transition-colors border-2 border-[var(--color-primary)] px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg whitespace-nowrap">
              Login
            </Link>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-12">
        ...
        <div className="mb-10 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-3 sm:mb-4">
            Reputation & <span className="text-[var(--color-secondary)]">Community</span>
          </h1>
          <p className="text-base sm:text-lg text-gray-600 max-w-md">
            Transparency drives the FreshSource ecosystem. Honest feedback from our network of farmers, buyers, and logistics partners.
          </p>
        </div>
            ...
            {reviews.map((review, i) => (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Avatar */}
                  <div className="w-12 h-12 bg-[var(--color-primary-dark)] rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
      ...
      <footer className="border-t border-gray-200 px-4 sm:px-6 md:px-10 py-8 sm:py-10 text-center text-sm text-gray-600 mt-12 sm:mt-16">
        <p className="font-bold text-gray-900 mb-2">FreshSource</p>
        <p>© 2026 FreshSource. Lagos, Lagos State, Nigeria.</p>
      </footer>
    </div>
  )
}
