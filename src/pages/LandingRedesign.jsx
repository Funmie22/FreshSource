  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[var(--color-background-warm)]">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-8 sm:px-8 md:pt-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:px-12 lg:pb-24">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} className="relative min-h-[430px] sm:min-h-[560px]"><div className="absolute inset-0 overflow-hidden rounded-[2rem] bg-[var(--color-primary-dark)]"><img src="/images/hero-nigeria-farming.jpg" alt="A Nigerian farmer working in a field" className="h-full w-full object-cover opacity-75 mix-blend-screen" /><div className="absolute inset-0 bg-gradient-to-t from-[var(--color-primary-dark)] via-transparent to-black/10" /></div><div className="absolute -bottom-5 left-4 right-4 rounded-2xl border border-white/20 bg-[var(--color-background-warm)]/95 p-4 shadow-2xl backdrop-blur sm:bottom-6 sm:left-6 sm:right-auto sm:w-72"><div className="flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--color-secondary-dark)]">Live route</p><span className="flex items-center gap-1.5 text-[10px] font-bold text-[var(--color-primary)]"><i className="h-2 w-2 animate-pulse rounded-full bg-[var(--color-primary)]" /> Active</span></div><p className="mt-3 font-[var(--font-heading)] text-2xl text-[var(--color-primary-dark)]">Lagos <span className="text-[var(--color-secondary)]">→</span> Abuja</p><div className="mt-3 flex items-center justify-between border-t border-black/10 pt-3 text-xs text-[var(--color-charcoal)]/60"><span>Next collection</span><strong className="text-[var(--color-charcoal)]">Today, 4:30 PM</strong></div></div><div className="absolute right-4 top-4 rounded-full bg-[var(--color-secondary)] px-4 py-2 text-xs font-bold text-white sm:right-6 sm:top-6">Harvest, not hype.</div></motion.div>
      </section>
