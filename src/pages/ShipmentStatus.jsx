...
function OrderTracking() {
  ...
  const flutterConfig = {
    ...
    tx_ref: order ? `FRESHSOURCE-${order.id}` : 'FRESHSOURCE-PENDING',
        ...
        {order.payment_status === 'pending' && (
          <div className="mb-10 bg-[var(--color-secondary-light)]/25 rounded-lg p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <p className="text-sm sm:text-base font-semibold text-[var(--color-secondary-dark)]">
              This order is awaiting payment — ₦{Number(order.total_price).toLocaleString()}
            </p>
            <button
            ...
            >
              {paymentProcessing ? 'Processing...' : `Pay ₦${Number(order.total_price).toLocaleString()}`}
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-lg sm:rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="font-[var(--font-heading)] font-bold text-lg text-[var(--color-secondary)]">₦{Number(order.total_price).toLocaleString()}</p>
              </div>
