...
function USSDListing() {
 ...
 const submitListing = async (finalLocation) => {
  ...
  const getSpokenText = () => {
    switch (step) {
      case 'menu':
        return 'Welcome to FreshSource. Press 1 to list new produce. Press 2 to check order status.'
      case 'crop':
        return `Select produce type. ${CROPS.map((c, i) => `Press ${i + 1} for ${c}`).join('. ')}.`
      case 'quantity':
        return `${crop} selected. Enter quantity in kilograms.`
      case 'price':
        return `${quantity} kilograms of ${crop}. Enter price per kilogram in Nigerian naira.`
      case 'location':
  ...
  const screenContent = () => {
    switch (step) {
      ...
      case 'submitting':
        return <p className="animate-pulse">Sending to FreshSource network...</p>
      case 'checkStatus':
        return (
          <p>
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-md mx-auto px-6 py-16">
        ...
        <p className="text-xs text-gray-500 text-center mt-2 mb-8">
          Simulates *123# — for farmers without smartphone access.
        </p>
        ...
        <p className="text-xs text-gray-400 text-center mt-6">
          This simulates a real USSD session (e.g. dialing *123#) for farmers without
          smartphone or internet access. Data submitted here goes directly into the same
          FreshSource database as the web app.
        </p>
