...
function ProduceListing() {
  ...
  const flutterConfig = {
    ...
    tx_ref: product ? `FRESHSOURCE-${product.id}` : 'FRESHSOURCE-PENDING',
    amount: product ? quantity * product.price_per_unit + getTransportCost(product.location) : 0,
    currency: 'NGN',
    payment_options: 'card,mobilemoney,ussd',
    redirect_url: `${window.location.origin}/payment-callback`,
    customer: {
      email: user?.email || 'buyer@freshsource.com',
      phonenumber: user?.phone || '+2348000000000',
      name: user?.name || 'FreshSource Buyer',
    },
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background-warm)]">
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-10 lg:gap-12 items-stretch">
          <div className="flex flex-col">
            ...
            <p className="text-base sm:text-lg text-[var(--color-charcoal)]/70 max-w-md leading-relaxed">
              Sun-cured, freshly harvested produce from the mineral-rich soils of the Lagos valley, grown using traditional cultivation techniques.
            </p>
            ...
            <div className="grid grid-cols-2 gap-6 mt-8 sm:mt-10">
              ...
              <div className="min-w-0">
                ...
                <p className="font-[var(--font-heading)] text-2xl sm:text-4xl md:text-5xl font-bold text-[var(--color-secondary)] truncate">₦{product.price_per_unit}</p>
              </div>
              <div>
            ...
            <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-6 sm:p-8">
              ...
              <div className="space-y-3 mb-8 pt-6 border-t border-black/10">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-[var(--color-charcoal)]">₦{subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[var(--color-charcoal)]/60">Logistics Fee ({product.location} → Delivery)</span>
                  <span className="font-bold text-[var(--color-charcoal)]">₦{logisticsFee.toFixed(2)}</span>
                </div>
      ...
      <footer className="border-t border-black/10 px-4 sm:px-6 md:px-10 py-8 sm:py-10 text-center text-sm text-[var(--color-charcoal)]/60 mt-12 sm:mt-16">
        <p className="font-bold text-[var(--color-charcoal)] mb-2">FreshSource</p>
