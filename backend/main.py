...
# Relative imports to work seamlessly when Uvicorn is executed inside the package
...
async def send_buyer_alerts(db: Session, item: Listing, farmer_phone: str) -> int:
    buyers = matching_buyers(db, item.location or "", farmer_phone)
    message = (
        f"FreshSource alert: {item.quantity} {item.unit} of {item.crop_type} is "
        f"available in {item.location} at NGN {item.price_per_unit}/{item.unit}."
    )
    for buyer in buyers:
        try:
            await send_whatsapp_message(buyer.phone, message)
...
def create_order(
    ...
    if stock_update.rowcount != 1:
        ...
        if available is None:
            raise HTTPException(status_code=404, detail="Inventory item not found")
        raise HTTPException(
            status_code=409, detail=f"Only {available}{item.unit} is available"
        )
