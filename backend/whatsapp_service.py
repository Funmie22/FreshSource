...
def validate_twilio_signature(url: str, params: dict[str, str], signature: str | None) -> bool:
    """Validate a Twilio webhook signature in production or when explicitly enabled."""
    is_prod = os.getenv("ENVIRONMENT", "").lower() == "production"
    should_validate = os.getenv("TWILIO_VALIDATE_SIGNATURE", str(is_prod)).lower() == "true"
    ...
    auth_token = os.getenv("TWILIO_AUTH_TOKEN")
    if not auth_token or not signature:
        logger.warning("Twilio signature validation failed: Missing auth token or X-Twilio-Signature header.")
        return False
...
def send_whatsapp_message(to: str, message: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.exception("Failed to send WhatsApp message to %s via Twilio API", to)
        raise
